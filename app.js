...
// Richtwaarden per gangbare portie. Bewust rond gemaakt: dit zijn schattingen,
// geen etiketwaarden, en een valse precisie van drie decimalen zou suggereren
// dat ze nauwkeuriger zijn dan ze zijn.
const STANDAARD = [
  { name: "Appel", portie: "1 middel", calories: 78, protein_g: 0.4, carbs_g: 20.6, fat_g: 0.3, meal_type: "snack" },
  { name: "Banaan", portie: "1 middel", calories: 107, protein_g: 1.3, carbs_g: 27, fat_g: 0.4, meal_type: "snack" },
  { name: "Gekookt ei", portie: "1 stuk", calories: 78, protein_g: 6.3, carbs_g: 0.6, fat_g: 5.3, meal_type: "breakfast" },
  { name: "Boterham volkoren", portie: "1 snee", calories: 90, protein_g: 3.5, carbs_g: 15, fat_g: 1.2, meal_type: "lunch" },
  { name: "Kaas 48+", portie: "1 plak", calories: 76, protein_g: 5, carbs_g: 0, fat_g: 6.3, meal_type: "lunch" },
  { name: "Pindakaas", portie: "1 el", calories: 92, protein_g: 3.8, carbs_g: 2, fat_g: 7.6, meal_type: "lunch" },
  { name: "Halfvolle melk", portie: "200 ml", calories: 92, protein_g: 7, carbs_g: 9.6, fat_g: 3, meal_type: "breakfast" },
  { name: "Magere kwark", portie: "150 g", calories: 82, protein_g: 15, carbs_g: 5.4, fat_g: 0.2, meal_type: "snack" },
  { name: "Koffie met melk", portie: "1 kop", calories: 20, protein_g: 1.2, carbs_g: 1.6, fat_g: 1, meal_type: "snack" },
  { name: "Handje noten", portie: "25 g", calories: 154, protein_g: 5, carbs_g: 3.3, fat_g: 13.5, meal_type: "snack" },
  { name: "Kipfilet", portie: "100 g gebakken", calories: 165, protein_g: 31, carbs_g: 0, fat_g: 3.6, meal_type: "dinner" },
  { name: "Rijst", portie: "150 g gekookt", calories: 195, protein_g: 4, carbs_g: 42, fat_g: 0.5, meal_type: "dinner" },
  { name: "Pasta", portie: "150 g gekookt", calories: 210, protein_g: 7.5, carbs_g: 42, fat_g: 1.2, meal_type: "dinner" },
  { name: "Aardappelen", portie: "200 g gekookt", calories: 174, protein_g: 4, carbs_g: 38, fat_g: 0.2, meal_type: "dinner" },
];

function chipHtml(item, index, bron) {
  const onder = item.portie ? `<em>${escapeHtml(item.portie)}</em>` : `<em>${Math.round(item.calories)} kcal</em>`;
  return `<button type="button" class="chip" data-bron="${bron}" data-index="${index}">
    <b>${escapeHtml(item.name)}</b>${onder}
  </button>`;
}

function tekenSnelkeuze() {
  $("chips-common").innerHTML = STANDAARD.map((x, i) => chipHtml(x, i, "standaard")).join("");

  // Wat je vaak eet is waardevoller dan welke lijst dan ook, dus die komt
  // bovenaan zodra er genoeg gelogd is om iets zinnigs te tonen.
  const geteld = new Map();
  for (const m of state.meals) {
    const sleutel = m.name.trim().toLowerCase();
  ...
  favorieten = [...geteld.values()].sort((a, b) => b.n - a.n).slice(0, 5);
  $("quick-favorites").hidden = favorieten.length === 0;
  $("chips-favorites").innerHTML = favorieten.map((x, i) => chipHtml(x, i, "favoriet")).join("");
}
...
function kiesSnel(e) {
  ...
  $("m-kcal").value = Math.round(item.calories);
  $("m-protein").value = round(item.protein_g, 1);
  $("m-carbs").value = round(item.carbs_g, 1);
  $("m-fat").value = round(item.fat_g, 1);
  if (item.meal_type) $("m-type").value = item.meal_type;
